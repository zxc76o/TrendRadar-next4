...
> X 热点（`x-hot`）由项目内置爬虫提供，来源于 [trends24.in](https://trends24.in/)，可通过 `x_trends.location` 配置不同地区（例如 `japan`、`united-states`、`worldwide`），无需额外操作。
...
```yaml
x_trends:
  location: "worldwide" # trends24 slug
  max_topics: 20
```
