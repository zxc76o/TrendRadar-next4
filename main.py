...
class DataFetcher:
    ...
    def fetch_x_trends(self, alias: str) -> Tuple[Optional[str], str, str]:
        ...
        url = self._build_trends_url(location)
        ...
        try:
            ...
            if not items:
                # 若无法解析且不是全局站点，尝试回退到首页
                if location not in ("", "worldwide"):
                    fallback_url = "https://trends24.in/"
                    print(f"X 热点解析为空，尝试回退到 {fallback_url}")
                    response = requests.get(
                        fallback_url, headers=headers, proxies=proxies, timeout=15
                    )
                    response.raise_for_status()
                    items = self._extract_x_trends_items(response.text)
    ...
    @staticmethod
    def _build_trends_url(location: str) -> str:
        """构造 trends24 访问 URL"""
        loc = (location or "").strip().strip("/")
        if loc and loc.lower() != "worldwide":
            return f"https://trends24.in/{loc}/"
